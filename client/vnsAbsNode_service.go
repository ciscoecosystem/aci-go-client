...
func (sm *ServiceManager) CreateFunctionNode(name string ,l4-l7_service_graph_template string ,tenant string , description string, vnsAbsNodeattr models.FunctionNodeAttributes) (*models.FunctionNode, error) {	
	rn := fmt.Sprintf("AbsNode-%s",name)
	parentDn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s", tenant ,l4-l7_service_graph_template )
	vnsAbsNode := models.NewFunctionNode(rn, parentDn, description, vnsAbsNodeattr)
	err := sm.Save(vnsAbsNode)
	return vnsAbsNode, err
}

func (sm *ServiceManager) ReadFunctionNode(name string ,l4-l7_service_graph_template string ,tenant string ) (*models.FunctionNode, error) {
	dn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s/AbsNode-%s", tenant ,l4-l7_service_graph_template ,name )    
	cont, err := sm.Get(dn)
	if err != nil {
		return nil, err
	}
...
func (sm *ServiceManager) DeleteFunctionNode(name string ,l4-l7_service_graph_template string ,tenant string ) error {
	dn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s/AbsNode-%s", tenant ,l4-l7_service_graph_template ,name )
	return sm.DeleteByDn(dn, models.VnsabsnodeClassName)
}

func (sm *ServiceManager) UpdateFunctionNode(name string ,l4-l7_service_graph_template string ,tenant string  ,description string, vnsAbsNodeattr models.FunctionNodeAttributes) (*models.FunctionNode, error) {
	rn := fmt.Sprintf("AbsNode-%s",name)
	parentDn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s", tenant ,l4-l7_service_graph_template )
	vnsAbsNode := models.NewFunctionNode(rn, parentDn, description, vnsAbsNodeattr)
...
func (sm *ServiceManager) ListFunctionNode(l4-l7_service_graph_template string ,tenant string ) ([]*models.FunctionNode, error) {

	baseurlStr := "/api/node/class"	
	dnUrl := fmt.Sprintf("%s/uni/tn-%s/AbsGraph-%s/vnsAbsNode.json", baseurlStr , tenant ,l4-l7_service_graph_template )
