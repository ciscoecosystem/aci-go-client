...
func (sm *ServiceManager) CreateRecurringWindow(name string, scheduler string, description string, nameAlias string, trigRecurrWindowPAttr models.RecurringWindowAttributes) (*models.RecurringWindow, error) {	
	rn := fmt.Sprintf(models.RntrigRecurrWindowP , name)
	parentDn := fmt.Sprintf(models.ParentDntrigRecurrWindowP, scheduler )
	trigRecurrWindowP := models.NewRecurringWindow(rn, parentDn, description, nameAlias, trigRecurrWindowPAttr)
...
func (sm *ServiceManager) UpdateRecurringWindow(name string, scheduler string, description string, nameAlias string, trigRecurrWindowPAttr models.RecurringWindowAttributes) (*models.RecurringWindow, error) {
	rn := fmt.Sprintf(models.RntrigRecurrWindowP , name)
	parentDn := fmt.Sprintf(models.ParentDntrigRecurrWindowP, scheduler )
	trigRecurrWindowP := models.NewRecurringWindow(rn, parentDn, description, nameAlias, trigRecurrWindowPAttr)
