...
func (sm *ServiceManager) ReadRelationfromaAbsNodetoanLDev(function_node string, l4l7ServiceGraphTemplate string, tenant string) (*models.RelationfromaAbsNodetoanLDev, error) {
	dn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s/AbsNode-%s/rsNodeToLDev", tenant, l4l7ServiceGraphTemplate, function_node)
	cont, err := sm.Get(dn)
	if err != nil {
		return nil, err
	}
...
func (sm *ServiceManager) DeleteRelationfromaAbsNodetoanLDev(function_node string, l4l7ServiceGraphTemplate string, tenant string) error {
	dn := fmt.Sprintf("uni/tn-%s/AbsGraph-%s/AbsNode-%s/rsNodeToLDev", tenant, l4l7ServiceGraphTemplate, function_node)
...
func (sm *ServiceManager) ListRelationfromaAbsNodetoanLDev(function_node string, l4l7ServiceGraphTemplate string, tenant string) ([]*models.RelationfromaAbsNodetoanLDev, error) {

	baseurlStr := "/api/node/class"
	dnUrl := fmt.Sprintf("%s/uni/tn-%s/AbsGraph-%s/AbsNode-%s/vnsRsNodeToLDev.json", baseurlStr, tenant, l4l7ServiceGraphTemplate, function_node)
